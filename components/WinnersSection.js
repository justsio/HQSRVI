        ...
        <div key={fadeKey} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 fade-transition">
          {visibleWinners.map((winner, index) => (
            <div
              ...
              className="bg-gradient-to-br from-primary-50 to-white rounded-2xl p-5 shadow-md border border-primary-100 hover:shadow-lg transition-shadow"
            >
            ...
            {Array.from({ length: totalBatches }).map((_, i) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors ${
                  i === currentBatch ? 'bg-primary-600' : 'bg-gray-300 hover:bg-gray-400'
                }`}
